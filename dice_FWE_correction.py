...
def mask_using_nan(data_file, mask_file, filename=None):
    ...
    return(filename)
def dice(cluster_ALE_file, voxel_ALE_file,
         cluster_ALE_file_neg=None, voxel_ALE_file_neg=None, 
         cluster_ALE_stat_file=None, voxel_ALE_stat_file=None, voxel_ALE_stat_file_neg=None, cluster_ALE_stat_file_neg=None,
         study=None
         ):

    cluster_ALE_file = mask_using_nan(cluster_ALE_file, cluster_ALE_stat_file)  
    voxel_ALE_file = mask_using_nan(voxel_ALE_file, voxel_ALE_stat_file)    
    if cluster_ALE_file_neg is not None:
        cluster_ALE_file_neg = mask_using_nan(cluster_ALE_file_neg, cluster_ALE_stat_file_neg)
    if voxel_ALE_file_neg is not None:
        voxel_ALE_file_neg = mask_using_nan(voxel_ALE_file_neg, voxel_ALE_stat_file_neg)
    

     # *** Obtain Dice coefficient for each combination of images
     # Comparison of replication analyses
    
    if voxel_ALE_file is not None:
        voxel_ALE_res_cluster_ALE_pos_dice = sorrenson_dice(cluster_ALE_file, voxel_ALE_file)
        voxel_ALE_res_cluster_ALE_neg_dice = sorrenson_dice(cluster_ALE_file_neg, voxel_ALE_file_neg)
   
    

        # *** Printing results
    if voxel_ALE_file is not None:
        print ("voxel_ALE/cluster_ALE positive activation dice coefficient = %.5f, %.0f, %.0f" % voxel_ALE_res_cluster_ALE_pos_dice)
    if cluster_ALE_file_neg is not None:
        print ("voxel_ALE/cluster_ALE negative activation dice coefficient = %.5f, %.0f, %.0f" % voxel_ALE_res_cluster_ALE_neg_dice)
    elif voxel_ALE_file_neg is not None:
        print ("voxel_ALE/cluster_ALE negative activation dice coefficient = %.5f, %.0f, %.0f" % voxel_ALE_res_cluster_ALE_neg_dice)
