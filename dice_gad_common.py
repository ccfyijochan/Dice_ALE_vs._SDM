...
def mask_using_nan(data_file, mask_file, filename=None):
    ...
    return(filename)
def dice(ALE_file, AES_file,
         ALE_file_neg=None, AES_file_neg=None, 
         ALE_stat_file=None, AES_stat_file=None, ALE_stat_file_neg=None,
         study=None
         ):

    ALE_file = mask_using_nan(ALE_file, ALE_stat_file)  
    AES_file = mask_using_nan(AES_file, AES_stat_file)    
    if ALE_file_neg is not None:
        ALE_file_neg = mask_using_nan(ALE_file_neg, ALE_stat_file_neg)
    if AES_file_neg is not None:
        AES_file_neg = mask_using_nan(AES_file_neg, AES_stat_file)
    

     # *** Obtain Dice coefficient for each combination of images
     # Comparison of replication analyses
    
    if AES_file is not None:
        AES_res_ALE_pos_dice = sorrenson_dice(ALE_file, AES_file)
        AES_res_ALE_neg_dice = sorrenson_dice(ALE_file_neg, AES_file_neg)
